...
class PIC(nn.Module):
    ...
    def _perform_jtc_correlation_batch(
        ...
        if M != 8 or N != 8:
            raise ValueError(
                f"This JTC implementation expects signal and kernel lengths of 8. Got M={M}, N={N}."
            )
        ...
        input_plane_batch_rolled = torch.roll(
            input_plane_batch, shifts=roll_amount, dims=-1
        )
    ...
    def forward(self, input, weights):
        ...
        orig_shape_dim2 = input_full.shape[
            2
        ]  # 8 (number of signals/kernels in this dimension)
        ...
        output_reshaped = correlation_output_batched.reshape(
            orig_shape_dim0, orig_shape_dim1, orig_shape_dim2, N
        )
...
for i, plane_size in enumerate(plane_size_values):
    for j, sep in enumerate(sep_values):
        ...
        print(
            f"Training complete for Plane size: {plane_size}, Sep: {sep}, Best accuracy: {best_acc:.2f}%"
        )
        with open(results_filename, "a") as f:
