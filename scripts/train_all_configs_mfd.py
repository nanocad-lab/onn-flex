...
Each run writes its outputs to a dedicated sub-directory under `runs_mfd/`
that matches the config’s base-name (e.g. `runs_mfd/config_ideal/`).
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Train all configured runs or run only pretrain tests (MFD runs)"
    )
    parser.add_argument(
        "--pretrain-tests-only",
        action="store_true",
